 ...
 *
 * perf: {
 *  1: 'http://jsperf.com/comparison-of-memoization-implementations/19'
 * }
 */
...
var bsCached = bs.cachedGlobal('bs'); // not as many cache hits with bs
...
function ba1(a) {
    ...
    if(time_diff > 0.1)
        console.log('ba1 time: %s', time_diff);
...
function e1(a, b) {
    ...
    time_diff = (time_end - time_start)/1000000;
    if(time_diff > 1) {
        console.log('e1 time: %s', time_diff);
        console.log('  ->  %s, %s', a, b);
    }
