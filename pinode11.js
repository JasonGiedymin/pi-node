...
function ba1(a) {
    ...
    if(time_diff > 0.1) {
        console.log('ba1 time: [%s], values:[%s]', time_diff, a.toString());
    }
...
function e1(a, b) {
    ...
    var m = tempMab.div(2);
    var z1 = bsCached(a, m); // cc
    Pam = z1[0];
    Qam = z1[1];
    Tam = z1[2];

    var z2 = bsCached(m, b); // cc
    Pmb = z2[0];
    Qmb = z2[1];
    Tmb = z2[2];
    ...
    time_diff = (time_end - time_start)/1000000;
    if(time_diff > 15) {
        console.log('e1 time:[%s], values:[%s],[%s]', time_diff, a.toString(), b.toString());
        return [Pab, Qab, Tab];
    }
    else {
        return [Pab, Qab, Tab];
...
function bs(a, b) {
    ...
    if (ba === 1) {
        ...
        var result;
        if(time_diff > 0.1) {
            console.log('bs time:[%s], values:[%s],[%s]', time_diff, a, b);
            result = ba1Cached(a);
            //console.log("<== Loops: %s", loops);
            return end();
        }
        else {
            result = ba1Cached(a);
        ...
        if(time_diff > 0.1) {
            console.log('bs time: %s', time_diff);
            return e1Cached(a, b);
        }
        else {
            return e1Cached(a, b);
...
module.exports = piChudnovskyBS;
module.exports.bs = bs;
module.exports.bsCached = bsCached;
module.exports.e1 = e1;
module.exports.e1Cached = e1Cached;
module.exports.ba1 = ba1;
module.exports.ba1Cached = ba1Cached;
